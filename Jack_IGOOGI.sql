...
def liquidity(typec):
    ...
    frame.reset_index(drop=True, inplace=True)
    frame['liq'] = frame['liq'].str.strip() #reindex 
    print 'By liquidity'
    
    frame.reindex([6, 5, 4, 3, 2, 1, 0]) #and then other things 
    return frame 
...
def market_width(typec):
    ...
    frame['pnl per share'] = frame['pnl'] / frame['shares']
    frame['abs pnl per share'] = frame['absPnl'] / frame['shares']
    return frame 
    
    
def stock_std_dev(st, pnl):
    #more like volatility of the stock rather than 
    #actual standard deviation moves 
...
if __name__ == '__main__':
    ...
    allOrders.head()
    allOrders['liqBucket'] = pd.cut(allOrders.liq, [1, 250, 500, 1000, 2000, 100000])
    allOrders.head()
    allOrders.groupby('liqBucket')['pnl'].describe()
    allOrders.head()
    allOrders[0]
    allOrders.columns
    allOrders['priceBucket'] = pd.qcut(allOrders.price, 4)
    allOrders.groupby(['priceBucket', 'liqBucket']).mean()
    #taking this data, it gives the, count, mean as well as stdev of the 
    allOrders.groupby(['priceBucket', 'liqBucket']).pnl.mean()
    allOrders.groupby(['priceBucket', 'liqBucket']).pnl.describe().T
